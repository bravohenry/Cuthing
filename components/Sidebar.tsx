...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const handleDeleteAssetItem = (e: React.MouseEvent, projectId: string, assetId: string) => {
    e.stopPropagation();
    if (confirm('Delete this asset?')) {
      onDeleteAsset(projectId, assetId);
    }
  };

  return (
    <div
      className={`h-full flex flex-col flex-shrink-0 z-30 relative bg-nothing-sidebar border-r border-nothing-border transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] will-change-width ${isCollapsed ? 'w-[72px] px-2' : 'w-[240px] px-4'
        }`}
    >
      ...
      <div className="h-24 flex items-end pb-6 shrink-0 overflow-hidden relative pl-2">
        <div className={`transition-all duration-500 absolute left-0 bottom-6 whitespace-nowrap ${isCollapsed ? 'opacity-0 translate-x-[-10px]' : 'opacity-100 translate-x-2'
          }`}>
          <h1 className="font-dot text-3xl text-nothing-black leading-none tracking-tight">Cuthing</h1>
        </div>

        {/* Collapsed Fallback */}
        <div className={`w-full flex justify-center items-end pb-1 transition-all duration-500 ${isCollapsed ? 'opacity-100 delay-200' : 'opacity-0 pointer-events-none'
          }`}>
      ...
      <div className="mb-6 shrink-0 flex justify-center">
        <button
          ...
          className={`group bg-nothing-black text-nothing-inverse hover:scale-[1.02] active:scale-95 transition-all duration-300 flex items-center justify-center overflow-hidden shadow-lg ${isCollapsed
        ...
        >
          ...
          <span className={`font-medium text-sm tracking-wide whitespace-nowrap transition-all duration-500 text-nothing-inverse ${isCollapsed ? 'w-0 opacity-0 overflow-hidden' : 'w-auto opacity-100 ml-2'
            }`}>
            NEW PROJECT
          </span>
        </button>
          ...
          return (
            <div key={project.id} className="flex flex-col mb-1">
              ...
              <div
                ...
                className={`group relative flex items-center h-14 px-2 rounded-xl cursor-pointer transition-all duration-200 z-10 ${isActive
              ...
              >
                ...
                {/* Text Label */}
                <div className={`flex-1 min-w-0 transition-all duration-500 overflow-hidden whitespace-nowrap flex flex-col justify-center ml-1 text-left ${isCollapsed ? 'opacity-0 w-0 hidden' : 'opacity-100 w-auto'
                  }`}>
                ...
                <div className="ml-5 pl-4 border-l-2 border-nothing-border/50 my-1 space-y-0.5 animate-in slide-in-from-top-2 duration-300">
                  {project.assets && project.assets.map(asset => (
                    <div
                      ...
                      className="flex items-center justify-between px-2 py-2 text-[10px] text-nothing-black/70 hover:bg-nothing-card/60 rounded-md cursor-pointer transition-colors group/file"
                      title={asset.name}
                    >
                      <div className="flex items-center gap-2 overflow-hidden">
                        <Film size={12} className="text-nothing-red group-hover/file:scale-110 transition-transform shrink-0" />
                        <span className="truncate font-medium">{asset.name}</span>
                      </div>
                      <button
                        onClick={(e) => handleDeleteAssetItem(e, project.id, asset.id)}
                        className="opacity-0 group-hover/file:opacity-100 text-nothing-grey hover:text-nothing-red transition-opacity"
                      >
                        <Trash2 size={10} />
                      </button>
                    </div>
                  ))}
                  <div className="flex items-center gap-2 px-2 py-2 text-[10px] text-nothing-black/70 hover:bg-nothing-card/60 rounded-md cursor-pointer transition-colors group/file" title={project.sequenceName || 'Main Sequence'}>
                    <FileJson size={12} className="text-nothing-grey group-hover/file:scale-110 transition-transform" />
                    <span className="truncate font-medium">{project.sequenceName || 'Main Sequence'}</span>
                  </div>
                </div>
              )}
            </div>
      ...
      <div className="mt-auto pt-6 pb-6 flex flex-col gap-2 border-t border-nothing-border/50 px-2">
        ...
        >
          ...
          <div className={`flex-1 overflow-hidden whitespace-nowrap transition-all duration-500 text-left ${isCollapsed ? 'w-0 opacity-0 hidden' : 'w-auto opacity-100'
            }`}>
