...
import { Segment, TranscriptItem, ChatMessage, AnalysisStatus, Project, Asset } from './types';
...
const hasConfiguredApiKey = () => {
  return Boolean(GeminiService.getApiKey());
};
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setProjects(prev => {
      ...
      // Debounce saving to local storage could be good, but for now direct save
      ProjectService.saveProjects(updated);
      return updated;
    });
  }, [transcript, segments, messages, visualDescription]);

  const loadProjectData = (project: Project) => {
    setTranscript(project.transcript || []);
    setSegments(project.segments || []);
    setMessages(project.messages || []);
    setVisualDescription(project.visualDescription || "");
    // Note: Video source cannot be restored from localStorage. 
    // We reset it, and user might need to re-import if they want to see the video.
    // But we keep the metadata.
    if (videoSrc) URL.revokeObjectURL(videoSrc);
    setVideoSrc(null);
    setDuration(0);
    setCurrentTime(0);
    setIsPlaying(false);
    setAnalysisStatus(project.transcript && project.transcript.length > 0 ? 'ready' : 'idle');
  };
  ...
  const handleUpdateProjectName = (name: string) => {
    setProjects(prev => {
      const updated = prev.map(p => p.id === activeProjectId ? { ...p, name } : p);
      ProjectService.saveProjects(updated);
      return updated;
    });
  };
  ...
  const handleDeleteAsset = async (projectId: string, assetId: string) => {
    // Delete from storage
    try {
      await StorageService.deleteFile(assetId);
    } catch (e) {
  ...
  const handleSelectAsset = async (asset: Asset) => {
    ...
    // If no URL (session expired), try to load from storage
    if (!src) {
      try {
        const blob = await StorageService.getFile(asset.id);
        if (blob) {
          src = URL.createObjectURL(blob);
          // Update asset with new URL in state (optional, but good for cache)
    ...
    if (src) {
      setVideoSrc(src);
      // Reset editor state for new video? 
      // For now, let's just switch the source. 
      // Ideally we should have per-asset transcript/segments, but that's a bigger refactor.
      // The user just wants to switch media.
      // We might want to clear transcript/segments if they don't match, 
      // but let's assume for now they might want to apply same edits or just view.
      // Actually, if we switch video, the transcript is likely invalid.
      // Let's warn or reset.
      if (confirm("Switching media will reset current analysis. Continue?")) {
        setTranscript([]);
        setSegments([]);
        setAnalysisStatus('idle');
        setVisualDescription("");
      ...
      alert("Media file not found (it may have expired). Please re-import.");
    }
  };
  ...
  const handleApiKeyChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    ProjectService.saveApiKey(key);
    setApiKeyError(!key && !hasConfiguredApiKey());
  };
      ...
      setProjects(prev => {
        const updated = prev.map(p => {
          if (p.id === activeProjectId) {
            ...
            // Save to storage
            StorageService.saveFile(assetId, file).catch(err => console.error("Failed to save file", err));
            ...
            return {
              ...p,
              name: p.name === 'Untitled Project' ? newName : p.name,
              assets: [...(p.assets || []), newAsset],
              sequenceName: p.sequenceName || 'Main Sequence'
            };
          }
          return p;
        });
        ...
        <div className="h-16 flex items-center justify-between px-6 shrink-0 mb-3 relative">
          ...
          <div className="absolute left-1/2 -translate-x-1/2 top-1/2 -translate-y-1/2 w-[300px] text-center group">
            <div className="relative">
              <input
                value={activeProject?.name || "Untitled Project"}
          ...
          <div className="flex items-center gap-4">
            {apiKeyError && (
              <button
                ...
                className="flex items-center gap-2 text-nothing-black text-[10px] px-3 py-1.5 bg-nothing-card hover:bg-nothing-black/5 rounded-full border border-nothing-border font-bold uppercase transition-colors cursor-pointer"
              >
        ...
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-nothing-card w-[500px] p-8 rounded-[2rem] shadow-2xl border border-nothing-border relative">
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-center justify-between pb-4 border-b border-nothing-border/50">
                <div>
                  <div className="font-bold text-sm text-nothing-black">Google AI Studio API Key</div>
                  <div className="text-xs text-nothing-grey mt-1">Enter your own API key to use the app</div>
                </div>
              </div>
              <div className="pb-4 border-b border-nothing-border/50">
                <input
                  type="password"
                  value={customApiKey}
                ...
                />
                <div className="text-[10px] text-nothing-grey mt-2">
                  Your key is stored locally in your browser. Get one at <a href="https://aistudio.google.com/" target="_blank" className="underline hover:text-nothing-black">aistudio.google.com</a>
                </div>
              </div>
